...
class GetActualExpenseUseCase @Inject constructor(
    private val expenseRepository: ExpenseRepository,
    @IoDispatchers private val dispatcher: CoroutineDispatcher,
) {
    suspend operator fun invoke(expenseId: Int): ActualExpenseModel {
        return withContext(dispatcher) {
            expenseRepository.getActualExpenseById(expenseId = expenseId)
        }
    }
}